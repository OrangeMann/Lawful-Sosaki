...
//MESSENGER/NOTE FUNCTIONS===================================
		...
		if ("Edit")
			...
			if (in_range(src, U) && loc == U)
				...
				if (mode == 1)
					note = sanitize_uni(rhtml_decode(n))
					notehtml = note
...
/obj/item/device/pda/proc/create_message(var/mob/living/U = usr, var/obj/item/device/pda/P)
	...
	if(useMS && useTC) // only send the message if it's stable
		if(useTC != 2) // Does our recipient have a broadcaster on their level?
			...
			return
		var/send_result = useMS.send_pda_message("[P.owner]","[owner]","[sanitize_uni(rhtml_decode(t))]")
		if (send_result)
			U << "ERROR: Messaging server rejected your message. Reason: contains '[send_result]'."
			return

		tnote.Add(list(list("sent" = 1, "owner" = "[P.owner]", "job" = "[P.ownjob]", "message" = "[sanitize_uni(rhtml_decode(t))]", "target" = "\ref[P]")))
		P.tnote.Add(list(list("sent" = 0, "owner" = "[owner]", "job" = "[ownjob]", "message" = "[sanitize_uni(rhtml_decode(t))]", "target" = "\ref[src]")))
		for(var/mob/M in player_list)
			if(M.stat == DEAD && M.client && (M.client.prefs.toggles & CHAT_GHOSTEARS)) // src.client is so that ghosts don't have to listen to mice
				if(istype(M, /mob/new_player))
					continue
				M.show_message("<span class='game say'>PDA Message - <span class='name'>[owner]</span> -> <span class='name'>[P.owner]</span>: <span class='message'>[sanitize_uni(rhtml_decode(t))]</span></span>")
		...
		if (prob(15)) //Give the AI a chance of intercepting the message
			...
			for(var/mob/living/silicon/ai/ai in mob_list)
				...
				if(ai.aiPDA != P && ai.aiPDA != src)
					ai.show_message("<i>Intercepted message from <b>[who]</b>: [sanitize_uni(rhtml_decode(t))]</i>")
		...
		log_pda("[usr] (PDA: [src.name]) sent \"[sanitize_uni(rhtml_decode(t))]\" to [P.name]")
		P.overlays.Cut()
