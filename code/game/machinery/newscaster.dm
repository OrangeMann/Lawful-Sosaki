...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["set_channel_name"])
			src.channel_name = cp1251_to_utf8(strip_html(input(usr, "Provide a Feed Channel Name", "Network Channel Handler", "")))
			...
			src.msg = cp1251_to_utf8(strip_html(input(usr, "Write your Feed story", "Network Channel Handler", "")))
			...
			src.channel_name = cp1251_to_utf8(strip_html(input(usr, "Provide the name of the Wanted person", "Network Security Handler", "")))
			while (findtext(src.channel_name," ") == 1)
				src.channel_name = copytext(src.channel_name,2,lentext(src.channel_name)+1)
			src.updateUsrDialog()

		else if(href_list["set_wanted_desc"])
			src.msg = cp1251_to_utf8(strip_html(input(usr, "Provide the a description of the Wanted person and any other details you deem important", "Network Security Handler", "")))
			while (findtext(src.msg," ") == 1)
				src.msg = copytext(src.msg,2,lentext(src.msg)+1)
			src.updateUsrDialog()
...
obj/item/weapon/newspaper/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/pen))
		if(src.scribble_page == src.curr_page)
			...
			s = cp1251_to_utf8(copytext(sanitize(s), 1, MAX_MESSAGE_LEN))
			if (!s)
				return
