...
///////Interactions!!///////
	if(href_list["interaction"])
		...
		var/isnude = ( !H.wear_suit && !H.w_uniform)
		var/isnude_p = ( !P.wear_suit && !P.w_uniform)
		var/ya = "&#255;"
...
//				message_admins(H, P, "teased")
			...
			if (H.loc == P.loc && isnude_p && isnude && haspenis && hasanus_p)
				if (H.erpcooldown == 0)
					if (H.potenzia > 0)
						H.fuck(H, P, "anal")
//						message_admins(H, P, "fucked in ass")
				else
...
mob/living/carbon/human/proc/cum(mob/living/carbon/human/H as mob, mob/living/carbon/human/P as mob, var/hole)
	...
	if (istype(H.species, /datum/species/xenos))
		...
		if (hole == "mouth" || H.zone_sel.selecting == "mouth")
			message = pick("кончает [P] в рот.", "целитс[ya] в лицо [P], стрел[ya]ет тугой струёй спермы, но промахиваетс[ya].", "разбрызгивает сем[ya] на лицо [P].", "кончает на пол.")
	...
	H.multiorgasms += 1
	if (H.multiorgasms == 1)
		msg_admin_attack("[H.name] ([H.ckey]) came on [P.name] ([P.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[H.x];Y=[H.y];Z=[H.z]'>JMP</a>)")
	H.erpcooldown = rand(40, 80)
...
//			message_admins(H, P, "licked")
		...
		if (prob(35))
			if (P.species.name == "Human")
				...
				message = pick("вводит два пальца в пушистую вагину [P].", "тычет пальцами [P].", "ласкает [P] пальчиками.", "нежно поглаживает промежность [P].", "погружает пальцы глубоко в [P], ласка[ya] её изнутри.", "изучает глубины [P].")

		if (H.lastfucked != P || H.lfhole != hole)
			H.lastfucked = P
			H.lfhole = hole
//			message_admins(H, P, "fingered")
		...
		if (H.species.name == "Human" || H.species.name == "Skrell" || H.species.name == "Arachna")
			...
			if (H.lastfucked != P || H.lfhole != hole)
				...
				H.lastfucked = P
				H.lfhole = hole
//				message_admins(H, P, "sucked")

		else if (H.species.name == "Unathi")
			message = pick("облизывает член [P].", "стимулирует член [P] [ya]зыком.", "трёт член [P] о свой [ya]зык.", "проталкивает член [P] себе в пасть, стара[ya]сь не зацепить его зубами.", "стимулирует член [P] [ya]зыком.")
			if (prob(35))
				message = pick("облизывает член [P], прикрыв глаза от удовольстви[ya].", "постанывает, прикрыв глаза, не вынима[ya] член [P] из пасти.", "ласкает член [P] [ya]зычком, придержива[ya] его рукой.", "облизывает член [P] по всей длине.", "погружает член [P] все глубже себе в пасть.", "кончиком [ya]зыка облизывает головку члена [P].", "плюёт на кончик члена [P] и проталкивает его себе в глотку.", "двигает головой взад-вперёд, стимулиру[ya] член [P].", "тщательно вылизывает член [P].", "зажмурившись, полностью заглатывает член [P].", "ласкает член [P], помога[ya] себе руками.")
			if (H.lastfucked != P || H.lfhole != hole)
...
//				message_admins(H, P, "sucked")
			...
			message = pick("облизывает член [P].", "стимулирует член [P] [ya]зыком.", "трёт член [P] о свой [ya]зык.", "проталкивает член [P] себе в глотку, стара[ya]сь не зацепить его клювом.", "стимулирует член [P] [ya]зыком.")
			if (prob(35))
...
//			message_admins(H, P, "fucked")
		...
		if (prob(35))
			if (P.species.name == "Human")
				...
				message = pick("трахает [P] в клоаку.", "всаживает член [P] в анальное кольцо по самые [ya]йца.", "разрывает [P] очко бешеными фрикци[ya]ми.")
			else if (P.species.name == "Tajara")
				message = pick("трахает [P] под хвост.", "всаживает член [P] в анальное кольцо по самые [ya]йца.", "долбит [P] под пушистый хвостик.", "разрывает [P] очко бешеными фрикци[ya]ми.")
			else if (P.species.name == "Skrell")
				message = pick("трахает [P] в клоаку.", "всаживает член [P] в анальное кольцо по самые [ya]йца.", "разрывает [P] очко бешеными фрикци[ya]ми.")
			else if (istype(H.species, /datum/species/xenos))
				message = pick("трахает [P] в анальный проход.", "всаживает член [P] в анальное кольцо по самые [ya]йца.", "разрывает [P] очко бешеными фрикци[ya]ми.")
			if (P.species.name == "Slime")
				message = pick("трахает [P] в задницу.", "всаживает член [P] в анальное кольцо по самые [ya]йца, л[ya]па[ya]сь в в[ya]зкой слизи.", "разрывает [P] очко бешеными фрикци[ya]ми.")

		if (H.lastfucked != P || H.lfhole != hole)
			message = pick(" безжалостно прорывает анальное отверстие [P].", "всаживает член [P] в очко.")
			H.lastfucked = P
			H.lfhole = hole
//			message_admins(H, P, "fucked in anus")
		...
		if (H.lastfucked != P || H.lfhole != hole)
			...
			H.lastfucked = P
			H.lfhole = hole
//			message_admins(H, P, "fucked in mouth")
...
mob/living/carbon/human/proc/sex_moan()
	var/ya = "&#255;"
